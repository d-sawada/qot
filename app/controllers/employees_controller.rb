class EmployeesController < ApplicationController
  ...
  def daily_index
    ...
    @employees.each do |emp|
      ...
      id = dayinfo.work_pattern_id || emp.work_template.pattern_id_of(@date)
      ...
      csv_row = emp.daily_index_row(ex_vals) +
  ...
  def monthly_index
    dayinfo_by_emp_id =
      Hash[@company.dayinfos.monthly(@date).map{ |d| [d.employee_id, d] }]
    csv_keys = employee_monthly_index_keys(@emp_exes.pluck(:name)) +
               dayinfo_monthly_keys
    @table_keys = %w(✔︎) + csv_keys + %w(詳細)
    csv_rows, @table_rows = [], []
    @employees.each do |emp|
      dayinfo = dayinfo_by_emp_id[emp.id] || Dayinfo.new
  ...
  def show
    ...
    if @list == "day"
      ...
      table_csv_keys = pattern_daily_keys + dayinfo_daily_keys
      table_opt_keys = ["申請登録"]
      row =
        (WorkPattern.find_by_id(@employee.work_template.pattern_id_of(@date)) ||
        WorkPattern.new).daily_index_row + dayinfo.daily_index_row
      table_csv_rows = [row]
      ...
      work_patterns =
        Hash[@company.work_patterns.map{|pattern| [pattern.id, pattern]}]
      table_csv_keys = ["日付"] + pattern_daily_keys + dayinfo_daily_keys
      ...
      (1..day_num).each do |i|
        ...
        table_csv_rows << [i] + pattern.daily_index_row +
                          dayinfo.daily_index_row
        table_opt_rows << [
  ...
  def new
    @employee = @company.employees.new(password: "password",
  ...
  def bulk_action
    if params[:employee].present?
      ...
      if params[:bulk_action] == "bulk_change_pattern"
        employees = Employee.where("id in (#{@ids.join(", ")})")
                      .preload(:dayinfos)
        employees.each do |emp|
  ...
  def bulk_create_requests
    ...
    ids = @ids.join(', ')
    emp_id_to_dayinfo =
      Hash[Dayinfo.left_joins(:employee)
             .where("dayinfos.date = ? and employees.id in (#{ids})", @date)
             .select("dayinfos.id, dayinfos.employee_id, " \
                     "dayinfos.start, dayinfos.end")
  ...
  private
    ...
    def employee_params
      params.require(:employee)
        .permit(:company_code, :no, :name, :email, :has_password,
                :password, :password_confirmation,
                emp_emp_status_attributes: [
                  :company_id, :employee_id, :emp_status_id
                ])
    end
end
